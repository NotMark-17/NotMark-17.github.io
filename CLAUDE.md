...
- `index.html`: the portfolio. Vanilla HTML/CSS/JS in one file, Lenis smooth scroll, editorial look (light grey `#ECECEC`, black `#1E1E1E`, tiny labels, huge type, tilted stacked image plates).
  ...
  - Theme toggle runs a dot-flood (canvas halftone wave from the button, `dotFlood` in the main script) before swapping; the About `.statement` letters thicken near the cursor via -webkit-text-stroke (no reflow; one sweep on touch); Oracle `.cert` cards tilt with a rainbow foil + glare (`--rx/--ry/--mx/--my/--foil`), auto-shimmer on touch. Mark approved these on 2026-09-25.
