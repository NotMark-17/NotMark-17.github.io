...
- `index.html`: the portfolio. Vanilla HTML/CSS/JS in one file, Lenis smooth scroll, editorial look (light grey `#ECECEC`, black `#1E1E1E`, tiny labels, huge type, tilted stacked image plates).
  ...
  - Theme toggle: the new theme spreads from the button as a clip-path circle on a same-document view transition (`theme-vt` class disables CSS transitions meanwhile; fallback is an instant switch). About `.statement`: three stroked copies (`.st-bold`, --k 1-3) revealed by radial masks that follow the cursor (only CSS vars change; one sweep on touch). Oracle `.cert`: tilt via --rx/--ry plus pre-painted `.foil`/`.glare` layers moved with translate3d, eased in rAF. Mark rejected the first versions as laggy (per-frame canvas dots, per-letter stroke, repainted gradients): keep effects compositor-friendly.
