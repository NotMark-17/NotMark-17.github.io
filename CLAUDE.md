...
- `index.html`: the portfolio. Vanilla HTML/CSS/JS in one file, Lenis smooth scroll. Dark only (black `#000`, ink `#EDEDED`, accent orange `#FF5B1F`); fonts Rajdhani (display, via `--serif`), DM Sans (text), DM Mono (labels). Inspired by curtisdesignr.me, but Mark removed the parts that felt copied: no coordinates, no orange stats/"folder box" section, no "Receipts"/CRT section, no photo glitch, no light theme, no recruiter mode, no About `.statement`. Don't bring those back.
  - Flow: intro (orange screen, % counter, grid closes in, breaks into pixels on a canvas; once per session, `?intro` forces it) → hero (`.hero` pinned inside `.hero-pin`: his dark black-and-white cut-out `mark.webp`, big "MARK / PATEL" that decodes in, bio, tag) → the manifesto lights up word by word as you scroll → work stage → About → Ask me → Contact ("Slide into my inbox." as cursor-scattering canvas dots).
  - Sound (`window.mpSound`, synthesized with Web Audio, no files): hover ticks, clicks. Switch in the HUD and mobile menu, stored in localStorage `mp-sound`; on by default for mouse devices. Browsers only allow it after the first click.
  - Custom cursor ring (mouse only), thin fixed grid lines with drifting orange dots.
  - GoatCounter analytics (`markpatel.goatcounter.com`) with a `?r=` per-person tag.
  - Performance rules (Mark reported lag twice): no mix-blend-mode, no animated filters, no animated masks, no per-frame canvas redraws of thousands of shapes, no always-on rAF loops. Scroll effects flip classes or transforms only while scrolling. Measure with a CDP trace before/after any new effect (whole-page scroll held 60fps at the last push).
